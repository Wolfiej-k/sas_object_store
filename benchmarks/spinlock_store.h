...
// Global-spinlock baseline: a single std::unordered_map guarded by one
// test-and-set spinlock. The hash table itself is sequential; concurrency is
// purely contention on the lock.
struct spinlock_store {
    ...
    void acquire() noexcept {
        while (lock_.test_and_set(std::memory_order_acquire)) {
            // pure spin; used only as a baseline for low-contention hot paths
        }
    }
