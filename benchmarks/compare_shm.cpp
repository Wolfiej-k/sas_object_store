...
int main() {
    ...
    if (arrived == n) {
        auto warmup_until = std::chrono::steady_clock::now() +
                            std::chrono::seconds(cfg.warmup_secs);
        auto measure_until =
            warmup_until + std::chrono::seconds(cfg.bench_secs);
    ...
    sas::bench::arch_worker_loop(
        my_idx, cfg, *shared->work_ptr, shared->store_ptr, shared->ops,
        shared->tlb_misses, warmup_until, measure_until);
    ...
    if (done == n) {
        ...
        double secs =
            std::chrono::duration<double>(std::chrono::seconds(cfg.bench_secs))
                .count();
        double tlb_per_op =
            total_ops > 0 ? double(total_tlb) / double(total_ops) : 0.0;
        sas::bench::arch_emit_throughput_json(
            "shm", n, double(total_ops) / secs, tlb_per_op);
    }
